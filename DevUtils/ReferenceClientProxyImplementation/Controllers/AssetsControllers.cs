...
//             cache.TryAdd(res, await System.IO.File.ReadAllBytesAsync($"{proxyConfiguration.Api.AssetCache.DiskCachePath}/{res}"));
//         else
//         {
//             if (!Directory.Exists(proxyConfiguration.Api.AssetCache.DiskCachePath)) Directory.CreateDirectory(proxyConfiguration.Api.AssetCache.DiskCachePath);
//             if (res.EndsWith(".map")) return NotFound();
//             Console.WriteLine($"[Asset cache] Downloading {"https://discord.com/assets/" + res} -> {proxyConfiguration.Api.AssetCache.DiskCachePath}/{res}");
//             try
//             {
...
//                     if (bytes.Length == 0)
//                     {
//                         Console.WriteLine($"[Asset cache] Cloudflare detected, retrying {"https://discord.com/assets/" + res} -> {proxyConfiguration.Api.AssetCache.DiskCachePath}/{res}");
...
//                     }
//                     
//                     if(proxyConfiguration.Api.AssetCache.DiskCache) await System.IO.File.WriteAllBytesAsync($"{proxyConfiguration.Api.AssetCache.DiskCachePath}/{res}", bytes);
...
//             if(!proxyConfiguration.Api.AssetCache.MemoryCache) cache.TryRemove(res, out _);
...
//     {
//         ClientPatchOptions patchOptions = proxyConfiguration.TestClient.DebugOptions.PatchOptions;
